...
class SlimeAgent(Agent):
    ...
    def multiply(self):
        ...
        print(self.pos, neighborhood)
        valid_cells = []
        
        # print(vars(self.model.grid))
        
        for position in neighborhood:

            agents = self.model.grid.get_neighbors(position, moore=True, include_center=True, radius=0)

            print(agents)
            print(self.model.grid.grid[position[0]][position[1]])
            if any(isinstance(agent, SlimeAgent) for agent in agents):
        ...
        for cell in valid_cells:
            chem_agent = self.model.grid.get_neighbors(cell, moore=True, include_center=True, radius=0)
            if chem_agent[0].chem >= chem:
                chem = chem_agent[0].chem
                opt_cell = cell
        try:
            new_pos = opt_cell
            ...
            new_pos = np.random.choice(valid_cells)

            
        new_slime = SlimeAgent(self.model.next_id(), self)
        self.model.grid.place_agent(new_slime, new_pos)
...
class SlimeModel(Model):
    ...
    def __init__(self, pop, width, height):
        ...
        self.grid = MultiGrid(width, height, torus=False)  # torus = True
